...
func main()
   ;ShellExecute("https://www.google.pl/search?q=hack+it&oq=hack+it&aqs=chrome..69i57j0l5.3029j0j9&sourceid=chrome&ie=UTF-8'")
   ;sleep(2000)
...
while $status="słucha"
   Sleep(520)
	  status()
	  WEnd
if $status="nie rozumie" or $status="sprawdź" Then
    $activeWindow = WinGetHandle("[ACTIVE]")
  ; SoundPlay($baza&"\nie_zrozumialam.wav",1)
   ControlClick($war,"","[CLASS:Chrome_RenderWidgetHostHWND]","left",1,77, 29)
...
; <><><>><><><><><><><><><><><><><><><><><><>WYKRYWANIE STATUSU<><><><><><><><><><><><>><><>
func status()
   If  MemoryReadPixel(	485 ,210 ,$war)=0x1155CC then
	  $status="nie rozumie"
   ElseIf  MemoryReadPixel(	179,190,$war)=0x777777  Then
	  	  $status="słucha"
	     elseif MemoryReadPixel(240 ,205 ,$war)=0x1A0DAB Then
	  $status="sprawdź"
	    elseif MemoryReadPixel(103 ,150 ,$war)=0xFBBC05 then
	  $status="czeka"
...
if stringleft ($wiadomosc,21)="Jak jest po angielsku" then
   ...
   EndIf
if StringLower(StringLeft($wiadomosc,6))="Napisz" and $wiadomosc<>"Napisz maila"then
   Sleep(1000)
...
Next
GUIDelete($gui)
case "włącz facebooka"
   send("{LSHIFT DOWN}")
   ShellExecute("chrome.exe", "www.facebook.com","","")
    send("{LSHIFT up}")
case "Jak wysoka jest Wieża Eiffla"
     SoundPlay($baza&"\eiffla.wav",1)
  case "Ile lat ma Statua Wolności"
	 SoundPlay($baza&"\statuawolnosci.wav",1)
  case "Gdzie leży Sosnowiec"
...
case "włącz notatnik"
  ...
  case "muszę zrobić notatki"
  run("notepad.exe")

  case "kółko i krzyżyk"
   tictac()
 case "zagrajmy w kółko i krzyżyk"
   tictac()
    case "grajmy w kółko i krzyżyk"
   tictac()
    case "Zagrajmy w kółko i krzyżyk"
   tictac()
    case "Grajmy w kółko i krzyżyk"
...
case "napisz maila"
  ...
  ; While ProcessExists("sendmail.exe")<>@error
	...
	;  guigetmsg()
	;  WEnd
  case "Zagraj coś na pianinie"
	 grajnapianinie()
	   case "zagraj coś na pianinie"
	 grajnapianinie()
	   case "Zagraj na pianinie"
	 grajnapianinie()
	 case "zagraj na pianinie"
	 grajnapianinie()

  Case "Dzięki za pomoc"
...
Sleep($b)
;end line
;o u i o r p o i e  o i u w  i u y w o
Send("{y}")
Sleep($a)
Send("{u}")
Sleep($a)
...
func tictac()
ShellExecute($baza&"\tictactoe.jar")
sleep(2000)
while WinExists ("TicTacToe")
guigetmsg()
WEnd
Sleep(1000)
while WinExists ("TicTacToe")
